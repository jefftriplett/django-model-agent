...
Everything that depends on the instance is resolved fresh on every request:

| Source | Evaluated |
|---|---|
| `DjangoModelCapability` current values | **every request** |
| Any capability's `get_instructions()` callable | **every request** |
| `@ModelAgent.system_prompt` / `@ModelAgent.instructions` methods | **every request** |
| `_instructions_template` | **every request** |
| `_system_prompts` / `_instructions` class attributes | once — they are constants |
| The field schema (names and types) | once — fields do not change between runs |

So a decorated method reading `self.instance` stays correct as the instance
changes underneath a long-lived agent:
...
class PlaceAgent(ModelAgent):
    ...
    def state_hint(self) -> str:
        return f"The state is {self.instance.state}."


agent = PlaceAgent(place)
await agent.run("...")            # "The state is draft."
place.state = "public"
await agent.run("...")            # "The state is public."
```

The same applies to a template with conditionals on instance state — it is
re-rendered per request, not baked in when the agent is built.

!!! note "Cost of a per-request callable"

    Anything in a decorated method or template runs on every model request, and
    a single `run()` may make several. Keep them cheap. If one needs a query,
    fetch it in a capability's `before_run` and have the callable read the
    result rather than querying each time.
